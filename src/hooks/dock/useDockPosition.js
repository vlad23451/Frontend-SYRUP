...
export const useDockPosition = () => {
  ...
  useEffect(() => {
    if (position.x == null || position.y == null) {
      ...
      if (edge === 'top' || edge === 'bottom')
        setPosition({ x: null, y: pos.y })

      else setPosition({ x: pos.x, y: null })
    }
  }, [computeEdgePosition, edge, position.x, position.y])
  ...
  useEffect(() => {
    ...
    const onEdgeChanged = (e) => {
      ...
      if (newEdge === 'top')
        setPosition({ x: null, y: 18 })

      if (newEdge === 'bottom')
        setPosition({ x: null, y: window.innerHeight - 18 })

      if (newEdge === 'left')
        setPosition({ x: 18, y: null })

      if (newEdge === 'right')
        setPosition({ x: window.innerWidth - 18, y: null })
    }
  ...
  useEffect(() => {
    ...
    const onPointerUp = (e) => {
      ...
      if (!overlapsModal && minDistance < 48) {
        ...
        if (nearEdge === 'right')
          snappedX = vw - elRect.width - 8

        if (nearEdge === 'top')
          snappedY = 8

        if (nearEdge === 'bottom')
          snappedY = vh - elRect.height - 8
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      if (edge === 'top' && position.x == null)
        setPosition({ x: null, y: 18 })

      if (edge === 'bottom' && position.x == null)
        setPosition({ x: null, y: window.innerHeight - 18 })

      if (edge === 'left' && position.y == null)
        setPosition({ x: 18, y: null })

      if (edge === 'right' && position.y == null)
        setPosition({ x: window.innerWidth - 18, y: null })
    }
  ...
  const styleDock = useMemo(() => {
    ...
    if (edge === 'bottom') {
      return position.x == null
        ...
        : { left: position.x,
            right: 'auto',
            top: position.y ?? (window.innerHeight - 18),
            bottom: 'auto',
            transform: 'none'
          }
    }
    ...
    if (edge === 'right') {
      return position.y == null
        ...
        : { left: position.x ?? (window.innerWidth - 18),
            right: 'auto',
            top: position.y,
            bottom: 'auto',
            transform: 'none' }
