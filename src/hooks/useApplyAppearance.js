...
export function useApplyPrimaryColor(colorScheme, colorSaturation) {
  useEffect(() => {
    ...
    root.style.setProperty('--color-btn-bg', `rgb(${rgb[0]}, ${rgb[1]}, ${rgb[2]})`)
    root.style.setProperty('--color-btn-bg-hover', `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, 0.9)`)
...
export function useApplyCustomTheme(customTheme) {
  useEffect(() => {
    const root = document.documentElement
    if (customTheme?.bg) root.style.setProperty('--color-bg', customTheme.bg)
    if (customTheme?.text) root.style.setProperty('--color-text', customTheme.text)
    if (customTheme?.primary) root.style.setProperty('--color-primary', customTheme.primary)
    if (customTheme?.button) root.style.setProperty('--color-btn-bg', customTheme.button)
    if (customTheme?.panel) root.style.setProperty('--panel-bg', customTheme.panel)
    if (customTheme?.border) root.style.setProperty('--color-border', customTheme.border)
  }, [customTheme])
}
...
export function useApplyBackground(backgroundMode, backgroundColor, backgroundGradient, backgroundImage, backdropBlur, transparency) {
  useEffect(() => {
    ...
    if (backgroundMode === 'solid') {
      ...
      body.style.background = backgroundImage ? `url(${backgroundImage}) center/cover no-repeat fixed` : ''
    }
    const root = document.documentElement
    root.style.setProperty('--backdrop-blur', String(backdropBlur))
    const alpha = Math.max(0.35, Math.min(0.85, Number(transparency) / 100))
    // Soft frosted premium panel (bluish, not pure white)
