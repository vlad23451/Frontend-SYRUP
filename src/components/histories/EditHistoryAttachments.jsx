/**
 * @fileoverview Компонент для редактирования вложений к истории
 * 
 * Функциональность:
 * - Просмотр текущих вложений истории
 * - Добавление новых файлов к истории
 * - Удаление существующих вложений
 * - Замена всех вложений
 * - Предпросмотр файлов
 */

import React, { useState, useRef } from 'react'
import { uploadFile, getMediaType, getFileIcon } from '../../services/mediaService'
import { addFilesToHistory, replaceHistoryFiles } from '../../services/historyService'
import '../../styles/edit-attachments.css'

const EditHistoryAttachments = ({ history, onUpdate, onClose, isOpen = false }) => {
  const [attachedFiles, setAttachedFiles] = useState(history.attached_files || [])
  ...
  const handleSave = async () => {
    ...
    try {
      const hasRemovedFiles = attachedFiles.length < (history.attached_files || []).length
      ...
      if (hasRemovedFiles) {
        ...
        await replaceHistoryFiles(history.id, allFileIds)
      } else if (newFiles.length > 0) {
        const newFileIds = newFiles.map(file => file.id)
        await addFilesToHistory(history.id, newFileIds)
      }
...
export default EditHistoryAttachments
