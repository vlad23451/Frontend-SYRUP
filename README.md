...
## Траблшутинг
- «Не уходит запрос на бэкенд» — проверьте `BASE_URL` и формат URL (`?` vs `/?`)
- «Модалка не двигается» — проверьте refs и состояние `open`
- «Иконка в доке не видна» — проверьте импорт и параметры SVG, перезапустите dev‑сервер

---

## Лицензия
Внутренний проект. Использование и распространение по договорённости с владельцем кода.
